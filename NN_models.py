...
class KKL_AutoEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        #-- encoder
        self.encoder = create_mlp(x_dim, z_dim, net_arch) # T^-1
        self._encoder = create_mlp(y_dim, z_dim, [v // 2 for v in net_arch]) # τ*
    ...
    def trajectories(self, ys, xs0=None):
        ...
        if xs0 is None:
            ...
            z_dyn = self._encode(ys[:, 0, :]) # Take the features of the first time step of each sample in ys
        else:
        ...
        for k in range(traj_len - 1):
            if self.A_frozen:
                with torch.no_grad():
                    z_dyn = self.z_next(z_dyn, ys[:, k, :]) # generate potential trajectory zs
            else:
